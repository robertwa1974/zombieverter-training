...
> **Known issue — recent units:** Some newer VCU boards do not automatically assign an IP address to your device via DHCP. If `192.168.4.1` does not load, manually set your computer's Wi-Fi adapter to a static IP of `192.168.4.2` (subnet `255.255.255.0`), then try again. Any address in the `192.168.4.x` range other than `.1` will work.
